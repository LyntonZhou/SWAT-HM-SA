...
ORGN     = x(cellfun(@(x) isequal(x, 'ORGN'), symbol))*str_OrgN;

str_labP = [ones(1,n_layers) zeros(1,10-length(sol_depth))];
LABP     = x(cellfun(@(x) isequal(x, 'LABP'), symbol))*str_labP;

str_orgP = str_OrgN;
ORGP     = x(cellfun(@(x) isequal(x, 'ORGP'), symbol))*str_orgP;
...
if isequal(hru_lnd, upper('Row Crops'))
    ...
    while feof(fid1)==0;
        L=L+1;
        line=fgets(fid1);
        if L==4 && par_f(cellfun(@(x) isequal(x, 'SOLN'), symbol))==1;
        ...
        elseif L==6 && par_f(cellfun(@(x) isequal(x, 'LABP'), symbol))==1;
            fprintf(fid2,'%s%12.3f%12.3f%12.3f%12.3f%12.3f%12.3f%12.3f%12.3f%12.3f%12.3f\r\n', ...
