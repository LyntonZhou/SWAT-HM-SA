...
while feof(fid1)==0;
    ...
    elseif L==16 && par_f(cellfun(@(x) isequal(x, 'BC1'), symbol))==1;
        fprintf(fid2,'%16.3f\t  %s\n',BC1,'| BC1:	Rate constant for biological oxidation of NH4 to NO2 in the reach at 20º C [day-1]');       
    elseif L==17 && par_f(cellfun(@(x) isequal(x, 'BC2'), symbol))==1;
        fprintf(fid2,'%16.3f\t  %s\n',BC2,'| BC2:	Rate constant for biological oxidation of NO2 to NO3 in the reach at 20º C [day-1]');
    elseif L==18 && par_f(cellfun(@(x) isequal(x, 'BC3'), symbol))==1;
        fprintf(fid2,'%16.3f\t  %s\n',BC3,'| BC3:	Rate constant for hydrolysis of organic N to NH4 in the reach at 20º C [day-1]');
    elseif L==19 && par_f(cellfun(@(x) isequal(x, 'BC4'), symbol))==1;
        fprintf(fid2,'%16.3f\t  %s\n',BC4,'| BC4:	Rate constant for mineralization of organic P to dissolved P in the reach at 20º C [day-1]');
